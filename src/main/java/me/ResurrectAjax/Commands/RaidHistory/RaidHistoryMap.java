...
public class RaidHistoryMap {
	...
	public void addBlocks(Integer blockID, String[] block) {
	...
	public HashMap<UUID, List<UUID>> getPartyByID(int ID) {
		...
		for(String[] party : parties) {
			if(Integer.parseInt(party[0]) == ID) {
	...
	public LinkedHashMap<Integer, ItemStack> getBlocksByRaidID(int raidID) {
		...
		for(int key : blocks.keySet()) {
			if(Integer.parseInt(blocks.get(key)[0]) == raidID) {
	...
	public HashMap<Integer, ItemStack> getItemsByContainerBlockID(int blockID) {
		HashMap<Integer, ItemStack> itemHash = new HashMap<Integer, ItemStack>();
		for(int key : containerItems.keySet()) {
			if(Integer.parseInt(containerItems.get(key)[0]) == blockID) {
				...
				if(containerItems.get(key)[2] != null) {
					meta.setDisplayName(containerItems.get(key)[2]);	
				}
				...
				if(containerItems.get(key)[4] != null) {
					for(String enchantString : containerItems.get(key)[4].split(";")) {
						...
						if(Enchantment.getByKey(NamespacedKey.minecraft(namedKey)) != null) {
	...
	public Map<Integer, String[]> getRaidsByIslandUUID(UUID islandUUID) {
		...
		for(int raidID : raids.keySet()) {
			if(UUID.fromString(getRaidByID(raidID)[2]).equals(islandUUID)) {
				raidMap.put(raidID, getRaidByID(raidID));
			}
		}
	...
	private boolean deleteRaidsOlderThan() {
		...
		if(configLoad.getBoolean("Database.RemoveData.Enabled")) {
			...
			if(days == null && months == null && weeks == null) {
				throw new IllegalArgumentException("Please use the right time formats(d|M): config.yml");	
			}	
			else {
				...
				if(db.deleteRaidOlderThan(dateStr)) {
					return true;
				}
			}
		}
